...
def ceshixiangqing(reslt, id, name, is_header, header, url, meth, yuqi, json, relust):
    xiangqing = '''
        <tr class="case-tr %s">
            <td>%s</td>
            <td>%s</td>
    ...
    ''' % (reslt, id, name, is_header, header, url, meth, yuqi, json, passfail(relust))
    return xiangqing
...
def relust(titles, starttime, endtime, passge, fail, id, name, is_header, header, url, meth, yuqi, json, relust, exceptions,
           weizhi):
    if type(name) == list:
        relus = ' '
        for i in range(len(name)):
            if relust[i] == "pass":
                ...
                clazz = 'error'
            relus += (
                ceshixiangqing(clazz, id[i], name[i], is_header[i], header[i], url[i], meth[i], yuqi[i], json[i], relust[i]))
        text = title(titles) + connent + shouye(starttime, endtime, passge, fail, exceptions,
                                                weizhi) + shanghai + relus + weibu
    else:
        text = title(titles) + connent + shouye(starttime, endtime, passge, fail, exceptions,
                                                weizhi) + shanghai + ceshixiangqing(id, name, is_header, header, url, meth,
                                                                                    yuqi, json, relust) + weibu
    return text


def createHtml(filepath, titles, starttime, endtime, passge, fail, id, name, is_header, header, url, meth, yuqi, json,
               relusts, exceptions, weizhi):
    texts = relust(titles, starttime, endtime, passge, fail, id, name, is_header, header, url, meth, yuqi, json, relusts,
                   exceptions, weizhi)
